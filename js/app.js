...
(function () {
  ...
  async function selectWatershed(watershedId) {
    ...
    UI.showLoading('interventionListBody' in {} ? 'interventionListBody' : 'interventionListBody');
  ...
  async function computeAllStatuses() {
    state.statusById = {};
    await Promise.all(state.interventions.map(async (iv) => {
      ...
      state.statusById[iv.intervention_id] = result.status;
    }));
  }
